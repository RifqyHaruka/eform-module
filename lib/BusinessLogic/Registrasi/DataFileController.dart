...
//CHECK ERROR DENGAN SERVICE BARU
  checkError() {
    if (Get.find<OtpController>().createAccountModelResponse?.errorCode !=
        null) {
      var errorCode =
  ...
  checkFileDone() async {
    prefs = Get.find<DataController>().prefs;
    if (prefs.getString("selfiePhoto") != null &&
        prefs.getString("selfiePhoto") != "") {
      isLivenessDone = true;
      selfiePhoto = prefs.getString("selfiePhoto");
    }

    if (prefs.getString("idSelfiePhoto") != null &&
        prefs.getString("idSelfiePhoto") != "") {
      isKtpDone = true;
      idSelfiePhoto = prefs.getString("idSelfiePhoto");
    }

    if (prefs.getString("idPhoto") != null &&
        prefs.getString("idPhoto") != "") {
      isNpwpDone = true;
      idPhoto = prefs.getString("idPhoto");
    }

    if (prefs.getString("signaturePhoto") != null &&
        prefs.getString("signaturePhoto") != "") {
  ...
  saveImageSize(
    ...
    prefs.setString(
        originalKey, "${(originalSize / 1024 / 1024).toStringAsFixed(2)}Mb");
    prefs.setString(
        compressedKey, "${(compressSize / 1024 / 1024).toStringAsFixed(2)}Mb");
  }
  ...
  Future<void> startLiveness(BuildContext context,
    ...
    try {
      ...
      ResultData? results =
          await FluttterActivePassiveLiveness.startLivenessDetection(config);
      ...
      if (results?.data != null) {
        ...
        Uint8List bytes =
            (await FlutterExifRotation.rotateImage(path: results!.image))
                .readAsBytesSync();

        try {
          /// compressing....
          Uint8List? selfiePhoto =
          ...
          var resulthasilLiveness = await Services()
              .POST(urlcekLiveness, 'Api Cek Liveness', body: bodycekLiveness);
          print(resulthasilLiveness?.statusCode);

          var resulthasilLivenessStatus =
              resulthasilLiveness?.data?.header['status'];

          if (resulthasilLiveness?.statusCode == 200 &&
              resulthasilLivenessStatus == true) {
            livenssResponseModel =
                LivenssResponseModel.fromJson(resulthasilLiveness?.data?.body);
            if (!isOnlyLiveness) {
              customLoading.dismissLoading();
              goSelfieKTPCamera();
            } else {
          ...
          var _resSelfie =
              await Services().POST(urlSavePhoto, 'Api Save Photo', body: body);
  ...
  goSelfieKTPCamera() async {
    if (Platform.isAndroid) {
      ...
      Get.toNamed(Routes().wajahktp)!
          .then((val) => val ? checkFileDone() : null);
    }
  }
